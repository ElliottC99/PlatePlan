/**
 * scripts/utils/ingredient-parse.js
 * PlatePlan Ingredient Line Parsing & Text Sectioning Engine
 */
...
(() => {
    ...
    try {
      ...
      return steps.map(step => {
        ...
        ings.forEach(ing => {
          ...
          const searchTerms = [...new Set([ing.rawName, (ing.name || '').toLowerCase(), base, singular, plural])].filter(t => t && t.length >= 3);
          ...
          text = text.replace(qtyPattern, (match, amountStr, unitStr, ingMention) => {
            ...
            if (u === 'kg' || u === 'l') amountInGrams = amount * 1000;
            ...
            const toGramsFn = window.toGrams || ((q) => q * 100);
            const totalGrams = ing.grams || toGramsFn(ing.qty, ing.unit);

            if (totalGrams > 0 && amountInGrams > 0 && unitStr) {
              let pct = Math.min(100, Math.max(1, Math.round((amountInGrams / totalGrams) * 100)));
              return `${pct}% of the ${ingMention}`;
            } else if (ing.qty > 0) {
              let pct = Math.min(100, Math.max(1, Math.round((amount / ing.qty) * 100)));
              return `${pct}% of the ${ingMention}`;
            }
            return match;
          });
  ...
  const normaliseMultiplierAndUnits = (rawLine) => {
    ...
    text = text.replace(/^(\d+(?:\.\d+)?\s*)?(?:a\s+)?small\s+bunch(?:es)?\b\s*(?:of\s+)?(.*)$/i, (m, countStr, remainder) => `${Math.round((countStr ? parseFloat(countStr) : 1) * 15 * 10) / 10}g ${remainder.trim()}`);
    text = text.replace(/^(\d+(?:\.\d+)?\s*)?(?:a\s+)?large\s+bunch(?:es)?\b\s*(?:of\s+)?(.*)$/i, (m, countStr, remainder) => `${Math.round((countStr ? parseFloat(countStr) : 1) * 30 * 10) / 10}g ${remainder.trim()}`);
    text = text.replace(/^(\d+(?:\.\d+)?\s*)?(?:a\s+)?bunch(?:es)?\b\s*(?:of\s+)?(.*)$/i, (m, countStr, remainder) => `${Math.round((countStr ? parseFloat(countStr) : 1) * 20 * 10) / 10}g ${remainder.trim()}`);
    text = text.replace(/^(\d+(?:\.\d+)?\s*)?(?:a\s+)?handful(?:s)?\b\s*(?:of\s+)?(.*)$/i, (m, countStr, remainder) => `${Math.round((countStr ? parseFloat(countStr) : 1) * 30 * 10) / 10}g ${remainder.trim()}`);
    ...
    if (compoundMatch) {
      ...
      let remainder = compoundMatch[4].trim().replace(/^(?:tins?|cans?|packs?|blocks?|pots?|jars?|bottles?)\s*(?:of\s+)?/i, '').trim();

      let totalWeight = count * subQty;
      let unit = 'g';

      if (rawUnit === 'kg' || ['l', 'litre', 'litres', 'liter', 'liters'].includes(rawUnit)) {
        totalWeight = count * subQty * 1000;
        unit = rawUnit === 'kg' ? 'g' : 'ml';
      } else if (rawUnit === 'ml') {
        totalWeight = count * subQty; unit = 'ml';
      } else if (['oz', 'ounce', 'ounces'].includes(rawUnit)) {
        totalWeight = Math.round(count * subQty * 28.35 * 10) / 10; unit = 'g';
      } else if (['lb', 'lbs', 'pound', 'pounds'].includes(rawUnit)) {
        totalWeight = Math.round(count * subQty * 453.6 * 10) / 10; unit = 'g';
      } else if (['floz', 'fl oz'].includes(rawUnit)) {
        totalWeight = Math.round(count * subQty * 30 * 10) / 10; unit = 'ml';
      } else if (['cup', 'cups'].includes(rawUnit)) {
        totalWeight = Math.round(count * subQty * 240 * 10) / 10; unit = 'ml';
      } else if (['tbsp', 'tablespoon', 'tablespoons'].includes(rawUnit)) {
        totalWeight = Math.round(count * subQty * 15 * 10) / 10; unit = 'g';
      } else if (['tsp', 'teaspoon', 'teaspoons'].includes(rawUnit)) {
        totalWeight = Math.round(count * subQty * 5 * 10) / 10; unit = 'g';
      } else if (['tin', 'tins', 'can', 'cans'].includes(rawUnit)) {
        totalWeight = count * subQty * 400; unit = 'g';
      }
      return `${totalWeight}${unit} ${remainder}`;
    }

    text = text.replace(/^(\d+(?:\.\d+)?)\s*(?:fl\.?\s*oz\.?|fluid\s*ounces?)\b\s*(?:of\s+)?(.*)$/i, (m, q, rem) => `${Math.round(parseFloat(q) * 30 * 10) / 10}ml ${rem.trim()}`);
    text = text.replace(/^(\d+(?:\.\d+)?)\s*(?:cups?)\b\s*(?:of\s+)?(.*)$/i, (m, q, rem) => `${Math.round(parseFloat(q) * 240 * 10) / 10}ml ${rem.trim()}`);
    text = text.replace(/^(\d+(?:\.\d+)?)\s*(?:oz|ounces?)\b\s*(?:of\s+)?(.*)$/i, (m, q, rem) => `${Math.round(parseFloat(q) * 28.35 * 10) / 10}g ${rem.trim()}`);
    text = text.replace(/^(\d+(?:\.\d+)?)\s*(?:lbs?|pounds?)\b\s*(?:of\s+)?(.*)$/i, (m, q, rem) => `${Math.round(parseFloat(q) * 453.6 * 10) / 10}g ${rem.trim()}`);
    text = text.replace(/^(\d+(?:\.\d+)?)\s*(?:tbsp|tablespoons?)\b\s*(?:of\s+)?(.*)$/i, (m, q, rem) => `${Math.round(parseFloat(q) * 15 * 10) / 10}g ${rem.trim()}`);
    text = text.replace(/^(\d+(?:\.\d+)?)\s*(?:tsp|teaspoons?)\b\s*(?:of\s+)?(.*)$/i, (m, q, rem) => `${Math.round(parseFloat(q) * 5 * 10) / 10}g ${rem.trim()}`);
  ...
  const parseIngredientLine = (raw) => {
    ...
    if (unit === 'handful') { qty = qty * 30; unit = 'g'; }
    else if (unit === 'bunch') { qty = qty * 20; unit = 'g'; }
    else if (unit === 'pinch' || unit === 'dash') { qty = qty * 1; unit = 'g'; }
    ...
    if ((liquidMeasureToMl.includes(unit) && isLikelyLiquidFn(name)) || unit === 'fl oz' || unit === 'floz') {
      qty = toGramsFn(qty, unit); unit = 'ml';
    } else if (liquidMeasureToMl.includes(unit) || volToG.includes(unit)) {
      qty = toGramsFn(qty, unit); unit = 'g';
    } else if (unit === 'l') {
      qty = qty * 1000; unit = 'ml';
  ...
  if (typeof window !== 'undefined') {
    window.normaliseUnicodeFractions = normaliseUnicodeFractions;
    window.parseRecipeNumber = parseRecipeNumber;
    window.normaliseLeadingQuantity = normaliseLeadingQuantity;
    window.cleanIngredientLinePrefix = cleanIngredientLinePrefix;
    window.splitPastedIngredientText = splitPastedIngredientText;
    window.getIngredientSectionHeading = getIngredientSectionHeading;
    window.splitPastedIngredientSections = splitPastedIngredientSections;
    window.splitPastedMethodText = splitPastedMethodText;
    window.escapeRegex = escapeRegex;
    window.ppEscapeRegex = ppEscapeRegex;
    window.convertMethodQuantitiesToPercentages = convertMethodQuantitiesToPercentages;
    window.detectStockIngredient = detectStockIngredient;
    window.normaliseMultiplierAndUnits = normaliseMultiplierAndUnits;
    window.parseIngredientLine = parseIngredientLine;
    window.parseIngredient = parseIngredientLine;
  }
})();
